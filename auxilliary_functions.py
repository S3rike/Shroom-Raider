...
def choose_map():
    ...
    while True:
        try:
            clear_screen()
            show_list_maps()
            commands = f'[[red]R[/red]] To Refresh List       [[yellow1]Q[/yellow1]] To Quit Game       [[green1]L[/green1]] To Display Leaderboard'
            console.print(Panel.fit(commands, title="Commands", title_align="left"))
...
def show_list_maps():
    ...
    console.print(Panel.fit(
        available_maps,
        border_style="grey42",
        title="[blink steel_blue3]Available Maps[blink steel_blue3]",
        title_align="left"
    ), justify="center")
...
def show_entire_map(session):
    ...
    for row in session.map:
        ...
        list_display.append(''.join(emoji_display))
    final_ui = '\n'.join(list_display)
...
def show_game_over(map, mushroom_count, mins_taken, seconds_taken, completion_time):
    ...
    console.print(f'However, you were able to collect [red]{mushroom_count["collected"]}[/red] mushrooms!\n')
    console.print(f'You played for [dim]{mins_taken} minutes and {seconds_taken}[not dim] seconds.')
    console.print(f'Full time: [blink]{format_completion_time(completion_time)}[/blink]\n')
    for row in map:
        emoji_display = [tile_ui.get(tile, tile) for tile in row]
...
def show_stage_clear(self, mins_taken, seconds_taken, completion_time):
    ...
    for line in display_stage_clear:
        print(line.center(column))
    console.print(f'You collected all [red]{self.mushroom_count['total']}[/red] mushrooms!\n')
    console.print(f'You played for [dim]{mins_taken} minutes and {seconds_taken}[not dim] seconds.\n')
    console.print(f'Full time: [blink]{format_completion_time(completion_time)}[/blink]\n')
    for row in self.map:
        emoji_display = [tile_ui.get(tile, tile) for tile in row]
